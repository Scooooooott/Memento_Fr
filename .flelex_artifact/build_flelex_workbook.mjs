...
if (!payloadPath || !outputPath || !previewDir) {
  throw new Error(
    "Usage: node build_flelex_workbook.mjs <payload.json> <output.xlsx> <preview-dir>",
  );
}

const payload = JSON.parse(await fs.readFile(payloadPath, "utf8"));
const columns = payload.columns;
const sheetOrder = payload.sheet_order ?? [
  "A1",
  "A2",
  "B1",
  "B2",
...
function columnWidth(column) {
  if (column === "frequency_rank") return 14;
...
for (const sheetName of sheetOrder) {
  ...
  sheet.tabColor = sheetName === "Unresolved" ? "#C65911" : "#1F4E78";
  ...
  header.format = {
    fill: sheetName === "Unresolved" ? "#C65911" : "#1F4E78",
  ...
  for (let index = 0; index < columns.length; index += 1) {
    ...
    if (numericColumns.has(columns[index]) && rows.length > 0) {
  ...
  if (rows.length === 0) {
    sheet.getRangeByIndexes(1, 0, 1, columns.length).values = [
      columns.map(() => null),
    ];
  }
  const table = sheet.tables.add(
    ...
    true,
    `Flelex_${sheetName.replace(/[^A-Za-z0-9]/g, "_")}`,
  );
